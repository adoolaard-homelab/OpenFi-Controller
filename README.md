...
**Implemented:** device pairing and inventory (`system.board`, `system.info`), Dashboard, Client Devices (`luci-rpc getDHCPLeases`), a single-level Topology view (adopted devices + their DHCP clients), wireless radio/SSID management and channelization (Radios, Settings → WiFi), automated 802.11s mesh backhaul setup across adopted devices (Mesh — joins selected radios to a shared Mesh ID/channel/key, swaps in a mesh-capable `wpad` build via `opkg` when needed, and surfaces peer-link status per radio), and the full page/navigation skeleton (Ports, Insights, Settings → System/Networks/DHCP & DNS/Firewall, and the per-device Overview/Insights/Settings/Tools drawer).

**Not implemented yet** (each has a place in the UI already): channel analysis (spectrum scan, recommended channel), enhanced mesh routing for multi-gateway/partial-cabled backhauls (batman-adv, mesh11sd, OLSR), switch port/VLAN configuration, interface CRUD (WAN/LAN/Guest/VLAN), DHCP static leases and custom DNS, firewall zones/port forwarding/traffic rules, routing/ARP/conntrack views, system log streaming and process management, system administration (hostname/timezone/NTP/SSH/root password), OPKG package management, cron/startup scripts, LED configuration, diagnostics (ping/traceroute/nslookup), backup/restore, and firmware updates. These will be built out incrementally, all through `uci` and other ubus RPC objects on the same session-authenticated connection already used for pairing.
